...
render() {
	...
	return <tr>
		...
		<td>{row.simId}</td><td>{row.simName}</td><td>{row.simDate}</td><td>{row.categ}</td><td>{row.descr}</td><td>{row.dttm}</td>
	</tr>;
}
//
}
	...
	return (<div><table className="noborder">
		<thead><tr>
			...
			<tr><td/><td/><td colSpan="2">
				...
				<td colSpan="2"><span style={spanStyle}>Row order:</span> 
				<select value={this.props.sortOrder} onChange={e => this.selChange(e)} >
					<option value="simName">by name</option>
					<option value="simDate">by date</option>
